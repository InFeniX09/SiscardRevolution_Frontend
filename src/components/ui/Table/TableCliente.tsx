...
import { TablaCliente } from "@/src/interfaces/tablaCliente.interface";
...
const INITIAL_VISIBLE_COLUMNS = ["cliente_id", "sDsCliente", "sDsNif", "sDsDireccion"];
export const columnsSolicitud = [
  { name: "cliente_id", uid: "cliente_id", sortable: true },
  { name: "sDsCliente", uid: "sDsCliente", sortable: true },
  { name: "sDsNif", uid: "sDsNif", sortable: true },
  { name: "sDsDireccion", uid: "sDsDireccion", sortable: true },
];
interface Props {
  array: TablaCliente[];
}
...
export default function TablaCliete({ array }: Props) {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (hasSearchFilter) {
      ...
      filteredUsers = filteredUsers.filter(
        (user) =>
          (user.cliente_id &&
            user.cliente_id.toLowerCase().includes(lowerCaseFilterValue)) ||
          (user.sDsCliente &&
            user.sDsCliente.toLowerCase().includes(lowerCaseFilterValue))
      );
    }
    if (
      statusFilter !== "all" &&
      ...
      filteredUsers = filteredUsers.filter((user) =>
        Array.from(statusFilter).includes(user.cliente_id)
      );
    }
  ...
  const renderCell = React.useCallback(
    (user: TablaCliente, columnKey: React.Key) => {
      const cellValue = user[columnKey as keyof TablaCliente];
